...
drop policy if exists "Users can create their own likes" on public.recipe_likes;
create policy "Users can create their own likes" on public.recipe_likes
  for insert with check (auth.uid() = user_id);

drop policy if exists "Users can delete their own likes" on public.recipe_likes;
...
as $$
  ...
  join public.user_follows uf on rd.user_id::uuid = uf.followed_id
